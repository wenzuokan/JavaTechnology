 ...
 * 作用：通过请求路劲匹配请求
 * value属性是数组类型，即当前浏览器所发送请求的请求路径匹配value属性中任何一个值
 * 则当前请求就会被注解所标识的方法进行处理
 * 3,@RequestMapping注解的method属性
 * 作用：通过请求的请求方式匹配请求
 * method属性是RequestMethod类型的数组，即当前浏览器所发送请求的请求方式匹配method属性中的任何一个
 * 若 浏览器所发送的请求路径和@RequestMapping注解value属性匹配，但是请求方式不匹配
 * 此时页面报错
 ...
 * ？：任意单个字符（不包括？）
 * *：任意个数的任意字符（不包括/和？）
 * **：任意层数的任意目录，注意使用方法只能**写在双斜线中，前后不能有任何其他字符
 * 7,@RequestMapping注解使用路径中的占位符
 * 传统：/deleteUser？id=1
 * rest：/user/delete/1
 * 需要在@RequestMapping注解的value属性中设置的路径中，使用{XXX}的方式表示路径中的数据
